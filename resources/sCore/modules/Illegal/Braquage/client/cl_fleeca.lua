...
function Fleeca:StartScalform()
    ...
    SneakyEvent("clp:removeitem", "drill", 0)
...
function Fleeca:OpenDoorFl(id)
    ...
    if getObjdoor and DoesEntityExist(getObjdoor) then
        ...
        SneakyEvent("cFleeca:OpenDoor", getObjdoor, doorpos)
    end
end
        ...
        while Fleeca.nFleeca ~= 0 do
            ...
            if Fleeca.moove == 1 then
                ...
                if disTance < 1.3 then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour utiliser votre ~b~perceuse.")
...
function Fleeca:StartFleecaHeist()
    ...
    -- if not Fleeca:PlayerHasBag() then return ESX.ShowNotification("~r~Vous devez avoir un sac.") end
...
AddEventHandler("cFleeca:BinginDrill", function()
    TriggerServerEvent("sCall:SendCallMsg", "Braquage de banque", GetEntityCoords(PlayerPedId()), "police", false)
    TriggerServerEvent("sCall:SendCallMsg", "Braquage de banque", GetEntityCoords(PlayerPedId()), "lssd", false)
    Fleeca:StartCallScal()
    SneakyEvent("cFleeca:SetCooldown", Fleeca.nFleeca.id)
end)
...
AddEventHandler("cFleeca:clientBankHeist", function(bool, _)
    if bool == 1 then
        Fleeca.nFleeca = _;
        SneakyEvent("cFleeca:GetCoolDown", Fleeca.nFleeca.id)
    elseif bool == 2 then
        if _ then
