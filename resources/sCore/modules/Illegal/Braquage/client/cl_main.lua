...
AddEventHandler('eSup:RobberyStart', function(i)
    if not IsPedDeadOrDying(eSup.Peds[i]) then
        ...
        while timer >= GetGameTimer() do
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(eSup.Peds[i]), true) >= 7.5 then
                ClearPedTasks(eSup.Peds[i])
                Wait(0)
                ReleaseScriptAudioBank("Alarms")
                StopSound(eSup.IsRobbing)
                RemoveTimerBar()
        ...
        if not IsPedDeadOrDying(eSup.Peds[i]) then
            ...
            if DoesEntityExist(cashRegister) then
                ...
                PlayAmbientSpeechWithVoice(eSup.Peds[i],"BUMP","A_F_M_BEACH_01_WHITE_FULL_01","SPEECH_PARAMS_FORCE",1)
            end
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                wait = 1
                                DrawText3D(GetEntityCoords(bag).x, GetEntityCoords(bag).y, GetEntityCoords(bag).z, "Appuyez sur ~b~E~s~ pour ~b~ramasser le sac~s~.", 12)
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #eSup.Peds do
                if IsPedDeadOrDying(eSup.Peds[i]) then
                    TriggerServerEvent('eSup:PedDeadRobbery', i)
                end
            end
            Wait(5000)
        end
                        ...
                        if not eSup.IsRobbing then
                            ...
                            if canRob == true then
                                ...
                                if chance >= 0 and chance <= 25 then
                                    ...
                                    PlaySoundFromCoord(eSup.IsRobbing, "Burglar_Bell", eSup.Braquage[i].coords, "Generic_Alarms", 0, 0, 0)
                                    PlaySoundFrontend(-1, "Object_Dropped_Remote", "GTAO_FM_Events_Soundset", 0)
                                    ActivatePhysics(eSup.Peds[i])
                                    eSup.BarUpdate = AddTimerBar("Menace :",{percentage = 0.0, bg = {100,0,0,255}, fg = {200,0,0,255}})
                                    PlayAmbientSpeechWithVoice(eSup.Peds[i], "SHOP_HURRYING", "MP_M_SHOPKEEP_01_PAKISTANI_MINI_01", "SPEECH_PARAMS_FORCE", 1)
                                    ...
                                    TriggerServerEvent("sCall:SendCallMsg", "Braquage de supérette", eSup.Braquage[i].coords, "police", false)
                                    TriggerServerEvent("sCall:SendCallMsg", "Braquage de supérette", eSup.Braquage[i].coords, "lssd", false)
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                while wait >= GetGameTimer() do
                                    Wait(0)
                                    ShowNotification('~r~Gruppe6 viens de passer, les caisses sont vides.')
                                    StopSound(eSup.IsRobbing)
                                    SetFakeWantedLevel(0)
                                end
                            else
                                ShowNotification("~r~Cette superette a déjà été braqué, le vendeur n'a plus rien.")
                                StopSound(eSup.IsRobbing)
                                SetFakeWantedLevel(0)
                                Wait(0)
                            end
