...
local deletionDistance = 100
local respawnDistance = 400
local populateOnReboot = false
local forgetOnDestroyed = true
local entityManagement = false
entityManagementDistance = respawnDistance + deletionDistance -- must be equal more than 350!
  ...
  for plate, data in pairs(self.vehicles) do
    if not DoesEntityExist(data.entity) then
      if data.pos then
        ...
        if closestDistance and closestDistance < 400 then
...
function PV:UpdateAllVehicleData()
  ...
  for plate, data in pairs(self.vehicles) do
    ...
    if DoesEntityExist(data.entity) then
      ...
      --[[   if forgetOnDestroyed and (tonumber(data.props.bodyHealth) == 0 or not data.props.tankHealth) then
          PV.ForgetVehicle(plate)
        end ]]
...
function PV:RunEntityMangement()
  ...
  for i = 1, #vehicles do
    ...
    if entity > 0 and DoesEntityExist(entity) then
      ...
      if closestDistance ~= nil and closestDistance > entityManagementDistance then
