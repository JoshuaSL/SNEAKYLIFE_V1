...
MySQL.ready(function()
    MySQL.Async.fetchAll("SELECT * FROM truck_inventory2",{},function(result)
        local data = nil
...
function loadInvent(plate)
    MySQL.Async.fetchAll("SELECT * FROM truck_inventory2 WHERE plate = @plate LIMIT 1",
        {
            ["@plate"] = plate
        },
        function(result)
...
function MakeDataStore(plate, cb)
    ...
    MySQL.Async.fetchAll(
        ...
        function(result)
            ...
            MySQL.Async.execute(
                "INSERT INTO truck_inventory2 (plate, data, owned) VALUES (@plate, '{}', @owned)",
                {
                    ["@plate"] = plate,
                    ["@owned"] = owned
                }
...
AddEventHandler("esx_inventoryhud_trunk:getSharedDataStore",function(plate, cb)
    GetSharedDataStore(plate,function(store)
        cb(store)
    end)
end)