		...
		if NetworkIsPlayerActive(player) then
			...
			if IsPedFatallyInjured(ped) and not isDead then
				...
				if killer == ped or killer == -1 then
					...
					SneakyEvent('baseevents:onPlayerDied', killertype, GetEntityCoords(ped, false))
					hasBeenDead = true
				else
					...
					SneakyEvent('baseevents:onPlayerKilled', killerid, {killertype = killertype, weaponhash = killerweapon, killerinveh = killerinvehicle, killervehseat = killervehicleseat, killervehname = killervehiclename, killerpos = GetEntityCoords(ped, false)})
					hasBeenDead = true
				end
			...
			if not hasBeenDead and diedAt ~= nil and diedAt > 0 then
				...
				SneakyEvent('baseevents:onPlayerWasted', GetEntityCoords(ped, false))
				hasBeenDead = true
