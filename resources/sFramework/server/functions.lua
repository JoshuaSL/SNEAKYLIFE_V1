...
function ESX.RegisterServerCallback(name, cb)
	...
	if callbackDebug then
		RconPrint("^2["..GetCurrentResourceName().."] ^0: Callback "..name.." ^2REGISTERED^7\n")
    end
end
...
function ESX.TriggerServerCallback(name, requestId, source, cb, ...)
    ...
    if ESX.ServerCallbacks[name] then
        ...
        if counter[source] >= 25 then
            DropPlayer(source, 'Trop de requettes ( ServerCallback ) ont été effectué')
            return
        end
        ESX.ServerCallbacks[name](source, cb, ...)
		if identifier ~= nil then
        	print('[ID : '..source.. ' Pseudo : '..GetPlayerName(source)..' License : '..identifier..' ] use => [' .. name .. '] ')
		end
    else
...
onSaved = {}
function onSavedPlayer(bool, identifier)
	local xPlayer = ESX.GetPlayerFromIdentifier(identifier)
	if xPlayer == nil then return print("aaaaaaa : none") end
	if not bool then
		--print("aaaaaa: Saved position player "..xPlayer.getName().." for enter the property")
		xPlayer.setLastPosition(xPlayer.getCoords())
	end
	onSaved[identifier] = bool
end

function ESX.SavePlayer(xPlayer, cb)
	local asyncTasks = {}
	if xPlayer then
		if onSaved[xPlayer.identifier] == true then
			xPlayer.setLastPosition(xPlayer.getCoords())
		end
		...
		Async.parallel(asyncTasks, function(results)
			print(('[^5Sauvegarde du joueur^7] %s^7'):format(xPlayer.getName()))
			if cb then
				cb()
			end
		end)
...
function ESX.SavePlayers(cb)
	...
	if #xPlayers > 0 then
		...
		Async.parallelLimit(asyncTasks, 8, function(results)
			print(('[^5Sauvegarde^7] %s player(s)'):format(#xPlayers))
	...
	for i = 1, #xPlayers, 1 do
		...
		if xPlayer then
			...
			if DoesEntityExist(plyPed) and xPlayer.positionSaveReady then
				...
				if onSaved[xPlayer.identifier] == true or onSaved[xPlayer.identifier] == nil and lastCoords ~= nil then
...
function ESX.UseItem(source, item)
	if ESX.UsableItemsCallbacks[item] then
		...
		print('[sFramework] : ' .. source .. 'tried to use item : ' .. item)
	end
end
