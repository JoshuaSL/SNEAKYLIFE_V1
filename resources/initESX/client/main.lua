...
AddEventHandler('Sneakyesx_addons_gcphone:call', function(data)
	...
	if data.message ~= nil and data.message ~= '' then
		...
		SneakyEvent('Sneakyesx_addons_gcphone:startCall', data.number, data.message, {
			x = coords.x,
			y = coords.y,
			z = coords.z
		})
...
function OpenMenu(submitCb, cancelCb, restrict)
	...
	TriggerEvent('Sneakyskinchanger:getData', function(components, maxVals)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'skin', {
			...
			DeleteSkinCam()
			TriggerEvent('Sneakyskinchanger:loadSkin', LastSkin)
			...
			TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
				...
				if skin[data.current.name] ~= data.current.value then
					TriggerEvent('Sneakyskinchanger:change', data.current.name, data.current.value)

					TriggerEvent('Sneakyskinchanger:getData', function(_, maxVals)
						for i = 1, #elements, 1 do
							local newData = {}
							newData.max = maxVals[elements[i].name]

							if elements[i].textureof ~= nil and data.current.name == elements[i].textureof then
								TriggerEvent('Sneakyskinchanger:change', elements[i].name, 0)
								newData.value = 0
							end
...
RegisterNetEvent('Sneakyesx_skin:openMenu')
AddEventHandler('Sneakyesx_skin:openMenu', function(submitCb, cancelCb)
	OpenMenu(submitCb, cancelCb, nil)
end)

RegisterNetEvent('Sneakyesx_skin:openRestrictedMenu')
AddEventHandler('Sneakyesx_skin:openRestrictedMenu', function(submitCb, cancelCb, restrict)
	OpenMenu(submitCb, cancelCb, restrict)
end)

RegisterNetEvent('Sneakyesx_skin:openSaveableMenu')
AddEventHandler('Sneakyesx_skin:openSaveableMenu', function(submitCb, cancelCb)
	OpenSaveableMenu(submitCb, cancelCb, nil)
end)

RegisterNetEvent('Sneakyesx_skin:openSaveableRestrictedMenu')
AddEventHandler('Sneakyesx_skin:openSaveableRestrictedMenu', function(submitCb, cancelCb, restrict)
